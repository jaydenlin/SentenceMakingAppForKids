...
//		}
	...
	private boolean IsAskingThenSetAnswers(){
		IsAsking=false;
		if(answer.matches(".*[要知道|想知道|不知道|不會|不確定|不瞭解|不了解|不懂|怎麼|如何|要怎樣|教我].*("+nounString+")*[怎麼|如何].*造句.*")){//ask for noun
			IsAsking=true;
			try {
				question.questionPhrase=getAskString(answer, nounString, 1);
				question.isAskingAdj=true;
				resetAnswers();
			} catch (AskStringNotFound e) {
				question.questionPhrase="";
				e.printStackTrace();
			}
		}else if(answer.matches(".*[要知道|想知道|不知道|不會|不確定|不瞭解|不了解|不懂|怎麼|如何|要怎樣|教我].*("+adjString+")*[怎麼|如何].*造句.*")){//ask for adj
			IsAsking=true;
			try {
				question.questionPhrase=getAskString(answer, adjString, 1);
...
//		String[] confusedWords = { "提示", "不知道", "不懂", "聽不懂", "不會", "怎麼", "造句",
//				"難", "爛", "教我", "教學", "換一題", "下一", "什麼", "啥麼", "甚麼" };
		isConfused = false;
...
//			}
//		}
		if(answer.matches("(.*提示|不知道|不懂|聽不懂|不會|怎麼|造句|難|爛|教我|教學|換一題|下一|什麼|啥麼|甚麼).*")){
			resetAnswers();
			isConfused=true;
		}
