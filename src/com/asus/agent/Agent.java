...
//		}
		Log.w(getClass().getSimpleName(), "handler size is "+Integer.toString(handlers.size()));
		//set callback to serialize the exec of handlers
		for(int i=0;i<handlers.size();i++){
			//if(i+1<handlers.size()){//may be outof range
				handlers.get(i).setNotifyDoneCallback(new NotifyDoneCallback(i) {
					
					@Override
					public void doNextHandler(JudgeEngine judgeEngine) {
						// TODO Auto-generated method stub
						Log.w(getClass().getSimpleName(), "do next handler handlerIndex is "+Integer.toString(handlerIndex));
						Log.w(getClass().getSimpleName(), "do next handler "+Integer.toString(handlerIndex)+" call "+Integer.toString(handlerIndex+1));
